...
CLASS lcl_xlsx_process_rates DEFINITION.
  PUBLIC SECTION.
    TYPES : BEGIN OF ts_rates,
            ...
            BEGIN OF ts_result,
              datasize TYPE i,
              records  TYPE STANDARD TABLE OF REF TO data WITH DEFAULT KEY,
            END OF ts_result,
...
*            tt_record TYPE STANDARD TABLE OF ts_record WITH DEFAULT KEY,
    ...
    CONSTANTS : BEGIN OF hkd_currency,
                  currency              TYPE waers VALUE 'HKD',
...
*    METHODS :
  PRIVATE SECTION.
    CONSTANTS : BEGIN OF hkd_cursor,
                  date_field TYPE string VALUE 'END_OF_DAY',
...
CLASS lcl_xlsx_process_rates IMPLEMENTATION.
  ...
  METHOD get_currencies_from_file.
    ...
    LOOP AT currency_range ASSIGNING FIELD-SYMBOL(<currency_range_line>).
      SPLIT <currency_range_line>-low AT '/' INTO from_currency to_currency.
      APPEND VALUE #(
             to_currency = to_currency
             from_currency = from_currency
             source_to_factor = lcl_xlsx_process_rates=>hkd_currency-source_factor_as_to
             source_from_factor = lcl_xlsx_process_rates=>hkd_currency-source_factor_as_from
           ) TO currencies .
    ENDLOOP.
...
*           io_row    = xco_cp_xlsx=>coordinate->for_numeric_value( lcl_xlsx_process_rates=>hkd_cursor-factor_from_line )
*       ).
...
*        "check from or to
*        IF <currency>-to_currency = lcl_xlsx_process_rates=>hkd_currency-currency.
*          cell->get_value(
...
*    DATA(cursor) = worksheet->cursor(
*        io_column = xco_cp_xlsx=>coordinate->for_alphabetic_value( lcl_xlsx_process_rates=>hkd_cursor-date_from_column )
*        io_row    = xco_cp_xlsx=>coordinate->for_numeric_value( lcl_xlsx_process_rates=>hkd_cursor-date_from_line )
*    ).
...
*    me->worksheet = worksheet.
  ...
  METHOD get_rates_from_file.
    ...
    LOOP AT currencies ASSIGNING FIELD-SYMBOL(<currency>).
      LOOP AT json-result-records ASSIGNING FIELD-SYMBOL(<record>).
        ASSIGN COMPONENT lcl_xlsx_process_rates=>hkd_cursor-date_field OF STRUCTURE <record>->* TO FIELD-SYMBOL(<date>).
        IF sy-subrc = 0.
*          cl_abap_datfm=>conv_date_ext_to_int(
*              EXPORTING
*                  im_datext = <date>->*
...
*          DATA exchangerate TYPE string.
          IF <currency>-from_currency = lcl_xlsx_process_rates=>hkd_currency-currency.
            ASSIGN COMPONENT <currency>-to_currency OF STRUCTURE <record>->* TO FIELD-SYMBOL(<exchange_rate>).
            IF sy-subrc = 0.
              rate->source_exchangerate = 1 / <exchange_rate>->*.
            ENDIF.
          ELSE.
            ASSIGN COMPONENT <currency>-from_currency OF STRUCTURE <record>->* TO <exchange_rate>.
            IF sy-subrc = 0.
              rate->source_exchangerate = <exchange_rate>->*.
            ENDIF.
          ENDIF.
  ...
  METHOD process.
    ...
    IF json-header-err_code <> '0000'.
      RAISE EXCEPTION TYPE zzcx_fi_001
        EXPORTING
          textid = VALUE #(
          msgid = 'ZZFI'
          msgno = '012'
          attr1 = json-header-err_msg
          ).
    ENDIF.
